...
- In-memory matching queues (who's waiting, who's paired) live in one Node.js process. Fine up to a few thousand concurrent users on one solid server; beyond that you need multiple server processes/machines with shared state (Redis) and a load balancer. Note the P2P model keeps this cheap: the server never relays or stores chat content, so its per-user load is tiny.
