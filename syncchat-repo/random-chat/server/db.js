// ============================================================================
// Postgres (via Supabase's Supavisor pooler) + Upstash Redis.
//
// P2P MODE: chat message CONTENT (text, voice messages, verification
// clips) no longer passes through or gets stored on this server — it
// travels directly between the two participants' devices over encrypted
// WebRTC data channels (see P2P_ARCHITECTURE.md). This module therefore
// now mostly stores identity/social metadata and a few content-free
// conversation counters (unread badges, last-activity type). The legacy
// message-reading functions are kept so conversations from before the P2P
// switch remain viewable.
// ============================================================================
...
async function initSchema() {
  await query(`
    ...
    -- P2P mode: message CONTENT lives exclusively on the participants'
    -- devices (exchanged over encrypted WebRTC data channels — the server
    -- never sees or stores it). The conversations table therefore carries
    -- its own content-free metadata so the chat list still works:
    --   unread_a / unread_b ......... per-side unread counters
    --   last_message_type/sender .... "Message" / "Voice message" preview
    ALTER TABLE conversations ADD COLUMN IF NOT EXISTS unread_a INTEGER NOT NULL DEFAULT 0;
    ALTER TABLE conversations ADD COLUMN IF NOT EXISTS unread_b INTEGER NOT NULL DEFAULT 0;
    ...
    -- "Delete for everyone" in P2P mode: the delete itself travels
    -- device-to-device, but we keep an ID-only tombstone here so a partner
    -- who was offline at the time still applies the deletion on their next
    -- resume. Contains message IDs only — never any content.
    ...
    CREATE INDEX IF NOT EXISTS idx_tombstones_conv ON message_tombstones(conversation_id);
  `);
...
// ---- P2P content-free conversation metadata ----
// The sender's device pings the server once per message sent (type only,
// never content) so the partner's chat list can show unread badges,
// ordering and a generic preview without the server ever seeing the text.
async function noteConversationActivity(convId, senderId, type) {
  ...
  await query(
    `UPDATE conversations SET
     ...
     WHERE id = $1`,
    [convId, safeType, senderId]
  );
  queueTouchConversation(convId, Date.now());
}

async function clearUnread(convId, userId) {
  await query(
    `UPDATE conversations SET
...
// ---- Message tombstones (IDs only — see schema comment) ----
async function addTombstone(convId, messageId) {
  await query(
    `INSERT INTO message_tombstones (conversation_id, message_id, created_at)
     VALUES ($1, $2, $3) ON CONFLICT (conversation_id, message_id) DO NOTHING`,
    [convId, messageId, Date.now()]
  );
}
async function getTombstones(convId) {
  const rows = await query(`SELECT message_id FROM message_tombstones WHERE conversation_id = $1`, [convId]);
  return rows.map(r => r.message_id);
}
async function pruneTombstones(olderThanMs) {
  await query(`DELETE FROM message_tombstones WHERE created_at < $1`, [Date.now() - olderThanMs]);
}
...
async function getUnreadCount(convo, viewerId) {
  // P2P mode keeps explicit per-side counters (content never touches the
  // server, so unread can't be derived from message rows anymore). Fall
  // back to legacy message rows for pre-P2P conversations.
  const counter = convo.user_a === viewerId ? convo.unread_a : convo.unread_b;
  if (Number(counter) > 0) return Number(counter);
...
async function deleteAllConversationsForUser(userId) {
  ...
  for (const c of convos) {
    ...
    await query(`DELETE FROM message_tombstones WHERE conversation_id = $1`, [c.id]);
...
async function deleteConversation(conversationId) {
  ...
  await query(`DELETE FROM message_tombstones WHERE conversation_id = $1`, [conversationId]);
...
module.exports = {
  ...
  noteConversationActivity, clearUnread,
  addTombstone, getTombstones, pruneTombstones,
