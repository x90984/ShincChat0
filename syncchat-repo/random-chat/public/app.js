...
// ============================================================================
// P2P ARCHITECTURE — chat content travels directly between the two devices
// over an encrypted WebRTC data channel (DTLS). The socket.io connection is
// only used for matchmaking, opaque WebRTC signaling, and (rarely) as an
// automatic fallback relay when a direct path can't be established. The
// server never sees or stores message content.
// ============================================================================

// Message IDs are generated on the sender's device (not by the server) so
// both sides can reference a message that no central server ever touched.
function newId() {
...
// Chat history lives on YOUR device, not in a central database. The server
// only keeps content-free metadata (who chatted with whom, unread counts).
const LocalStore = (() => {
  const DB_NAME = 'syncchat_local';
  const STORE = 'messages';
  const MAX_PER_CONVO = 1000;
  let dbPromise = null;
    ...
    dbPromise = new Promise((resolve, reject) => {
      ...
      req.onupgradeneeded = () => {
        if (!req.result.objectStoreNames.contains(STORE)) {
          req.result.createObjectStore(STORE, { keyPath: 'convId' });
        }
      };
  ...
  async function getRec(convId) {
    try {
      const db = await open();
      return await new Promise((resolve, reject) => {
        const q = db.transaction(STORE, 'readonly').objectStore(STORE).get(convId);
        q.onsuccess = () => resolve(q.result || { convId, messages: [] });
        q.onerror = () => reject(q.error);
      });
    } catch { return { convId, messages: [] }; }
  }
  ...
  return {
    async list(convId) {
      if (!convId) return [];
      const rec = await getRec(convId);
      return rec.messages.slice().sort((a, b) => a.ts - b.ts);
    },
    async add(convId, msg) {
      if (!convId || !msg) return;
      const rec = await getRec(convId);
      if (rec.messages.some(m => m.id === msg.id)) return;
      rec.messages.push(msg);
      if (rec.messages.length > MAX_PER_CONVO) {
        rec.messages = rec.messages.sort((a, b) => a.ts - b.ts).slice(-MAX_PER_CONVO);
      }
      await putRec(rec);
    },
    async remove(convId, id) {
      if (!convId) return;
      const rec = await getRec(convId);
      const before = rec.messages.length;
      rec.messages = rec.messages.filter(m => m.id !== id);
      if (rec.messages.length !== before) await putRec(rec);
    },
    async replace(convId, messages) {
      if (!convId) return;
      await putRec({ convId, messages });
    },
    async clear(convId) {
      if (!convId) return;
      try {
...
// Merge the device-local history with any legacy (pre-P2P) rows the server
// still has, de-duplicated by id — this is what resume/history renders.
async function mergedHistory(convId, serverMessages) {
  const local = await LocalStore.list(convId);
  const byId = new Map();
  for (const m of (serverMessages || [])) byId.set(m.id, m);
  for (const m of local) {
    if (!byId.has(m.id)) {
      byId.set(m.id, {
        id: m.id, type: m.type, text: m.text || null, audio: m.audio || null,
        ts: m.ts, mine: !!m.mine, deleted: !!m.deleted, replyTo: m.replyTo || null
      });
    }
  }
...
// IDs already rendered this pairing — guards against any double-delivery
// when a message arrives over the P2P channel AND the fallback relay.
const renderedMessageIds = new Set();
let pendingSeenIds = []; // partner messages we've seen but not yet acked over the P2P channel

// ---- P2P data channel state ----
let dc = null;                    // the RTCDataChannel content flows over
let p2pInitiator = false;         // matched payload tells one side to make the offer
let matchRtcConfig = null;        // ICE servers handed to us at match time (STUN/TURN)
const inflightTransfers = new Map(); // chunked binary transfers (voice msgs, verify clips)
const P2P_CHUNK_SIZE = 16384;     // base64 chars per data-channel frame

function transportOpen() {
  return !!(dc && dc.readyState === 'open');
}

function sendP2PJson(obj) {
  if (!transportOpen()) return false;
  try { dc.send(JSON.stringify(obj)); return true; } catch (e) { return false; }
}

// Chunked binary-ish transfer over the data channel (voice messages and
// verification clips are too large for a single frame on some stacks).
function sendBlobP2P(kind, dataUrl, meta = {}) {
  if (!transportOpen()) return false;
  try {
    ...
    const id = newId();
    const n = Math.ceil(b64.length / P2P_CHUNK_SIZE);
    sendP2PJson({ t: 'bs', id, kind, mime, n, ...meta });
    for (let i = 0; i < n; i++) {
      sendP2PJson({ t: 'bc', id, i, d: b64.slice(i * P2P_CHUNK_SIZE, (i + 1) * P2P_CHUNK_SIZE) });
    }
    sendP2PJson({ t: 'be', id });
    return true;
...
function completeInboundTransfer(id) {
  const tr = inflightTransfers.get(id);
  inflightTransfers.delete(id);
  if (!tr || tr.got !== tr.chunks.length) return;
  const dataUrl = `data:${tr.mime};base64,${tr.chunks.join('')}`;
  if (tr.kind === 'voice') {
    receiveVoiceMessage({ id: tr.meta.msgId, ts: tr.meta.ts, audio: dataUrl });
  } else if (tr.kind === 'verify') {
    if (chatMode === 'video') return;
    showVerifyClip(dataUrl, false);
  }
}

function handleP2PFrame(frame) {
  if (!frame || typeof frame.t !== 'string') return;
  switch (frame.t) {
    case 'chat':
      receiveChatMessage({ id: frame.id, ts: frame.ts, text: frame.text, replyTo: frame.replyTo || null });
      break;
    case 'seen':
      handleSeenAck(Array.isArray(frame.ids) ? frame.ids : []);
      break;
    case 'deleted':
      if (typeof frame.id === 'string') applyDeletion(frame.id, false);
      break;
    case 'bs': {
      const n = Math.max(1, Math.min(100000, Number(frame.n) || 1));
      inflightTransfers.set(frame.id, {
        kind: frame.kind, mime: frame.mime || 'application/octet-stream',
        meta: { msgId: frame.msgId, ts: typeof frame.ts === 'number' ? frame.ts : Date.now() },
        chunks: new Array(n).fill(null), got: 0
      });
      break;
    }
    case 'bc': {
      const tr = inflightTransfers.get(frame.id);
      if (tr && Number.isInteger(frame.i) && frame.i >= 0 && frame.i < tr.chunks.length && tr.chunks[frame.i] === null) {
        tr.chunks[frame.i] = String(frame.d || '');
        tr.got++;
      }
      break;
    }
    case 'be':
      completeInboundTransfer(frame.id);
      break;
  }
}
...
function wireDataChannel(channel) {
  dc = channel;
  dc.onopen = () => { /* direct encrypted path to the partner is live */ };
  dc.onclose = () => { if (dc === channel) dc = null; };
  dc.onmessage = (event) => {
    try { handleP2PFrame(JSON.parse(event.data)); } catch { /* malformed frame — ignore */ }
  };
}

// The sender's device removes its local copy of an on_view (seen-then-
// vanish) message once the partner's device confirms it was displayed.
function handleSeenAck(ids) {
  if (currentDisappearingMode !== 'on_view' || !activeConversationId) return;
  ids.forEach(id => LocalStore.remove(activeConversationId, id));
}
...
function applySwitchMode(toMode, initiator) {
  ...
  if (toMode === 'video' && !wasVideo) {
    ...
    // Attach media to the (already existing, data-channel-carrying) peer
    // connection and show the blurred preview + Continue/Skip gate, same
    // as an initial video match. Keep the chat overlay hidden until
    // Continue is clicked, same as a fresh match — otherwise it would sit
    // on top of and block the preview's Continue/Skip buttons.
    ...
    // Moving into text: drop the media tracks but KEEP the peer connection
    // alive — its data channel is what text chat flows over in P2P mode.
    if (pc) {
...
function sendMessage() {
  ...
  if (!text) return;
  msgInput.value = '';
  const replyTo = pendingReplyTo ? { id: pendingReplyTo.id, text: pendingReplyTo.text } : null;
  clearReplyBar();
  // The id is minted right here on the device — the message below travels
  // device-to-device over the encrypted data channel and never touches a
  // central server.
  const msg = { id: newId(), ts: Date.now(), text, replyTo };
  addMessage(text, 'self', { id: msg.id, replyTo });
  renderedMessageIds.add(msg.id);
  myMessageCount++;
  trackVideoUnlock();
  LocalStore.add(activeConversationId, { id: msg.id, ts: msg.ts, type: 'text', text, replyTo, mine: true });
  if (transportOpen()) {
    sendP2PJson({ t: 'chat', ...msg });
  } else {
    // Direct path not (yet) available — relay fallback (content still only
    // transits, never stored, on the server).
    socket.emit('chat-message', msg);
  }
  socket.emit('conv-activity', { type: 'text' });
}

// Shared inbound path — used both for messages arriving over the direct
// P2P data channel and for the rare relay-fallback delivery.
function receiveChatMessage({ id, ts, text, replyTo }) {
  if (!id || renderedMessageIds.has(id)) return;
  renderedMessageIds.add(id);
  addMessage(text, 'partner', { id, replyTo });
  partnerMessageCount++;
  trackVideoUnlock();
  // on_view messages are shown but never written to this device's history.
  if (currentDisappearingMode !== 'on_view') {
    LocalStore.add(activeConversationId, {
      id, ts: typeof ts === 'number' ? ts : Date.now(), type: 'text', text, replyTo: replyTo || null, mine: false
    });
  }
  pendingSeenIds.push(id);
  maybeMarkSeen();
  if (!currentPartnerMuted) playSound('message');
}

socket.on('chat-message', (payload) => receiveChatMessage(payload));
...
// ---- Matching events ----
socket.on('matched', ({ roomId, initiator, partnerGender, resumed, conversationId, partnerId, partnerMuted, rtcConfig }) => {
  isSearchingForPartner = false;
  currentPartnerId = partnerId || null;
  currentPartnerMuted = !!partnerMuted;
  matchRtcConfig = rtcConfig || null; // server-provided ICE servers (STUN + optional TURN)
  ...
  // Establishes the pairing's peer connection (the encrypted device-to-
  // device path for everything — data channel first, media tracks on top
  // in video mode), with the matched payload's initiator flag deciding
  // which side sends the WebRTC offer.
  startPeerConnection(initiator, false);
  if (chatMode === 'video') {
...
// If the two browsers can't establish a direct P2P media connection
// (strict NATs/firewalls — covered if the server is configured with a
// TURN relay via the TURN_* env vars; without one, STUN-only can fail),
// pc.ontrack simply never fires and the video would otherwise sit blurred
// forever with no explanation and nothing to click. This timeout catches
// that and swaps in a clear "couldn't connect" state with a Skip button
// instead of a silent, permanent hang. (Text chat is unaffected either
// way — it auto-falls back to the relay path when the P2P channel can't
// be established.)
...
// ---- WebRTC ----
// One peer connection per pairing, created the moment two people are
// matched — even in plain text mode. Its data channel is the direct,
// encrypted path all chat content takes; audio/video tracks are layered
// onto the SAME connection when voice or video mode activates (via
// renegotiation), so text chat keeps flowing uninterrupted throughout.
const DEFAULT_RTC_CONFIG = {
  iceServers: [{ urls: 'stun:stun.l.google.com:19302' }]
};

function currentRtcConfig() {
  return matchRtcConfig || DEFAULT_RTC_CONFIG;
}

// Which media kinds the current UI state calls for.
function desiredMediaKinds() {
  if (chatMode === 'video') return ['audio', 'video'];
  if (voiceActive) return ['audio'];
  return [];
}

function createPeerConnectionCore() {
  pc = new RTCPeerConnection(currentRtcConfig());

  // The matched-initiator opens the data channel; the other side receives
  // it via ondatachannel. Either way, this is the pipe content flows over.
  if (p2pInitiator) {
...
function addLocalTracks(kinds) {
  if (!pc || !localStream) return false;
  let added = false;
  for (const kind of kinds) {
    const alreadySending = pc.getSenders().some(s => s.track && s.track.kind === kind && s.track.readyState === 'live');
    if (alreadySending) continue;
    const tracks = kind === 'audio' ? localStream.getAudioTracks() : localStream.getVideoTracks();
    if (!tracks.length) continue;
    const sender = pc.addTrack(tracks[0], localStream);
    if (kind === 'video' && sender.setParameters) {
...
function removeLocalTracks(kind) {
  if (!pc) return false;
  let removed = false;
...
// Bring attached media in line with `kinds`; returns whether anything changed.
function syncMediaTracks(kinds) {
  if (!pc) return false;
  let changed = false;
  if (addLocalTracks(kinds)) changed = true;
  for (const kind of ['audio', 'video']) {
    if (!kinds.includes(kind) && removeLocalTracks(kind)) changed = true;
  }
  return changed;
}
...
async function sendOffer() {
  ...
  try {
    ...
    if (!pc || pc.signalingState !== 'stable') return; // pairing changed mid-flight
...
// channel, just without a video track attached. In P2P mode this also runs
// for plain text matches: the connection (and its data channel) is what
// carries the chat itself.
function startPeerConnection(initiator, audioOnly) {
  p2pInitiator = initiator;
  if (!pc) createPeerConnectionCore();

  const kinds = (chatMode === 'video' && !audioOnly) ? ['audio', 'video']
    : (audioOnly || voiceActive) ? ['audio'] : [];
  syncMediaTracks(kinds);
...
socket.on('webrtc-offer', async ({ sdp }) => {
  if (!pc) createPeerConnectionCore();
  try {
    // Glare: both sides offered at once (e.g. a voice call and a mode
    // switch racing). The answerer side always wins — roll back ours.
    if (pc.signalingState === 'have-local-offer') {
      await pc.setLocalDescription({ type: 'rollback' });
    }
    // Make sure our wanted media is attached before answering, in case the
    // partner's offer raced ahead of our own track attach.
    syncMediaTracks(desiredMediaKinds());
...
socket.on('webrtc-answer', async ({ sdp }) => {
  if (pc && pc.signalingState === 'have-local-offer') {
    try { await pc.setRemoteDescription(new RTCSessionDescription(sdp)); } catch (e) { console.error(e); }
  }
});
...
// Renegotiate after tracks changed (voice call start/stop, video switch).
// Either side may offer; glare is resolved in the offer handler above.
...
// Detach just the audio track (renegotiating so the partner stops sending
// theirs too) while the peer connection itself — and the data channel that
// carries text chat — stays up.
function tearDownVoiceMedia() {
  if (pc) {
    removeLocalTracks('audio');
    renegotiate();
  }
...
function recordVerifyClip() {
  ...
  recorder.onstop = () => {
    ...
    reader.onload = () => {
      // Clip goes straight to the partner's device (transient — never
      // stored in anyone's history, on either side).
      if (transportOpen()) {
        sendBlobP2P('verify', reader.result);
      } else {
...
// Relay-fallback inbound for verify clips (the P2P path arrives via the
// data channel's 'verify' transfer kind instead).
...
function startVoiceMessageRecording() {
  ...
  voiceRecorder.onstop = () => {
    ...
    const reader = new FileReader();
    reader.onload = () => {
      // Device-minted id + direct device-to-device transfer (chunked over
      // the encrypted data channel); socket relay only as fallback.
      const msg = { id: newId(), ts: Date.now(), audio: reader.result };
      addVoiceMessage(msg.audio, true, { id: msg.id });
      renderedMessageIds.add(msg.id);
      LocalStore.add(activeConversationId, { id: msg.id, ts: msg.ts, type: 'voice', audio: msg.audio, mine: true });
      if (transportOpen()) {
        sendBlobP2P('voice', msg.audio, { msgId: msg.id, ts: msg.ts });
      } else {
        socket.emit('voice-message', msg);
      }
      socket.emit('conv-activity', { type: 'voice' });
    };
    reader.readAsDataURL(blob);
  };
  voiceRecorder.start();
...
// Shared inbound path for voice messages (P2P data channel + relay fallback).
function receiveVoiceMessage({ id, ts, audio }) {
  if (chatMode === 'video') return;
  if (!id || renderedMessageIds.has(id)) return;
  renderedMessageIds.add(id);
  addVoiceMessage(audio, false, { id });
  if (currentDisappearingMode !== 'on_view') {
    LocalStore.add(activeConversationId, {
      id, ts: typeof ts === 'number' ? ts : Date.now(), type: 'voice', audio, mine: false
    });
  }
  pendingSeenIds.push(id);
  maybeMarkSeen();
...
socket.on('voice-message', (payload) => receiveVoiceMessage(payload));
...
msgActionMenu.addEventListener('click', (e) => {
  ...
  else if (btn.dataset.act === 'delete-me') applyDeletion(id, true);
  else if (btn.dataset.act === 'delete-everyone') {
    applyDeletion(id, false);
    // Tell the partner's device directly; the server only gets the message
    // ID (tombstone) so the deletion also applies if they're offline.
    if (transportOpen()) sendP2PJson({ t: 'deleted', id });
    socket.emit('delete-message', { messageId: id, mode: 'everyone' });
  }
  closeMsgActionMenu();
});

// Apply a deletion to this device: DOM bubble + local history store.
function applyDeletion(messageId, onlyForMe) {
  if (!messageId) return;
  LocalStore.remove(activeConversationId, messageId);
...
document.getElementById('selDeleteBtn').addEventListener('click', () => {
  ...
  selectedMessageIds.forEach(id => applyDeletion(id, true)); // "delete for me" is purely device-local
  exitSelectionMode();
});

// Partner deleted a message for everyone — arrives either over the direct
// P2P data channel (applyDeletion called from handleP2PFrame) or via this
// relay path (offline-tombstone application / fallback transport).
socket.on('message-deleted', ({ messageId, onlyForMe }) => {
  applyDeletion(messageId, !!onlyForMe);
});
...
function clearCurrentChat() {
  ...
  fetch(`/api/history/${convId}/clear`, { method: 'POST', headers: authHeaders() })
    .then(r => r.json())
    .then(() => {
      LocalStore.clear(convId); // this device's copy goes too
      messagesEl.innerHTML = '';
...
function renderReviewsPartner() {
  ...
  // Past messages = this device's copy merged with legacy server rows.
  reviewsMessagesList.innerHTML = '<div class="reviews-messages-empty">Loading...</div>';
  mergedHistory(data.conversationId, data.messages).then(merged => {
    if (reviewsPartnersData[reviewsCurrentIndex] !== data) return; // paged away meanwhile
    if (merged.length) {
      reviewsMessagesList.innerHTML = merged.map(m => `
...
// ---- Read receipts: the socket ping only clears the server's content-free
// unread counter (chat-list badge); the actual "you've seen these message
// ids" receipt goes directly to the partner's device over the data channel,
// which is also what lets their device vanish on_view messages. ----
function maybeMarkSeen() {
  if (!document.getElementById('chatScreen').classList.contains('active')) return;
  socket.emit('mark-seen');
  if (pendingSeenIds.length && transportOpen()) {
    sendP2PJson({ t: 'seen', ids: pendingSeenIds });
    pendingSeenIds = [];
  }
}
...
function previewFor(lastMessage) {
  ...
  // P2P mode: the server only knows a text message happened, never its
  // content — legacy conversations still carry their real preview text.
...
document.getElementById('deleteAllChatsBtn').addEventListener('click', async () => {
  ...
  try {
    ...
    for (const item of allChatsCache) LocalStore.clear(item.conversationId); // wipe device copies too
...
socket.on('conversation-deleted', ({ conversationId }) => {
  LocalStore.clear(conversationId); // partner wiped it — drop our device copy as well
...
async function resumeConversation(item) {
  ...
  try {
    ...
    // History = this device's own copy merged with any legacy (pre-P2P)
    // rows the server still holds. Tombstones (ID-only deletion markers)
    // and the 24h timer are applied on top.
    const tombstones = new Set(data.tombstones || []);
    const merged = await mergedHistory(item.conversationId, data.messages);
    const expireBefore = currentDisappearingMode === '24h' ? Date.now() - 24 * 60 * 60 * 1000 : null;
    const kept = [];
    merged.forEach(m => {
      if (tombstones.has(m.id)) m.deleted = true;
      if (expireBefore && !m.deleted && m.ts <= expireBefore) m.deleted = true;
      if (m.deleted) {
    ...
    // Persist the tombstone/expiry outcomes back into the device store.
    LocalStore.replace(item.conversationId, kept.map(m => ({
      id: m.id, ts: m.ts, type: m.type, text: m.deleted ? null : m.text,
      audio: m.deleted ? null : m.audio, mine: !!m.mine, replyTo: m.replyTo || null, deleted: !!m.deleted
    })));
  } catch (e) {
