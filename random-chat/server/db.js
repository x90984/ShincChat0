// ============================================================================
// Postgres data layer. Talks to any Postgres (Supabase's Supavisor pooler,
// the bundled docker-compose instance, or your own). Caching goes through
// ./cache.js (local Redis → Upstash → in-process LRU) instead of a hard
// dependency on Upstash — see the top of cache.js for why.
// ============================================================================
require('dotenv').config(); // loads .env locally; on hosts, real env vars are used and this is a harmless no-op
...
if (!process.env.DATABASE_URL) {
  throw new Error('DATABASE_URL is not set. Locally docker-compose provides it; on Supabase use the "Transaction" pooler connection string (port 6543).');
}
...
const PG_SSL_DISABLED = process.env.PG_SSL === 'false'; // local docker Postgres needs no TLS
const pool = new Pool({
...
// ---- Cache-aside layer (local Redis → Upstash → in-process LRU) ----
const { cacheGet, cacheSet, cacheDel } = cacheLayer;
...
async function flushTouches() {
  ...
  // Sort by conversation id: every worker takes row locks in the same
  // order, so concurrent flushes from multiple workers can't deadlock.
  const batch = Array.from(pendingTouches.entries()).sort((a, b) => (a[0] < b[0] ? -1 : a[0] > b[0] ? 1 : 0));
...
// Async scrypt — the sync version blocked the event loop for ~50ms per
// login, which at scale stalls every other connection on the worker.
...
async function verifyPassword(password, salt, hash) {
  try {
    ...
    return false; // corrupt/missing hash — never crash the login route
  }
}
...
/**
 * The /api/history payload in ONE query (the old route did 3 queries per
 * conversation — partner lookup, last message, unread count — which is
 * 60+ round-trips for a 20-conversation list).
 */
async function listConversationsDetailed(userId) {
  const rows = await query(
    `SELECT c.id AS conversation_id, c.created_at, c.last_message_at,
            other_u.id AS partner_id, other_u.username AS partner_username,
            other_u.display_name AS partner_display_name, other_u.photo AS partner_photo,
            other_u.gender AS partner_gender, other_u.is_banned AS partner_banned,
            other_u.is_premium AS partner_premium, other_u.bio AS partner_bio,
            other_u.youtube_link AS partner_youtube, other_u.birth_date AS partner_birth_date,
            other_u.phone_hash AS partner_phone_hash,
            lm.type AS lm_type, lm.text AS lm_text, lm.ts AS lm_ts, lm.sender_id AS lm_sender_id,
            lm.deleted_for_everyone AS lm_deleted,
            (SELECT COUNT(*) FROM messages m WHERE m.conversation_id = c.id
               AND m.sender_id != $1 AND m.seen_at IS NULL AND m.deleted_for_everyone = 0) AS unread
     FROM conversations c
     LEFT JOIN users other_u
       ON other_u.id = CASE WHEN c.user_a = $1 THEN c.user_b ELSE c.user_a END
     LEFT JOIN LATERAL (
       SELECT m.* FROM messages m WHERE m.conversation_id = c.id AND m.deleted_for_everyone = 0
       ORDER BY m.ts DESC LIMIT 1
     ) lm ON true
     WHERE c.user_a = $1 OR c.user_b = $1
     ORDER BY COALESCE(lm.ts, c.last_message_at, c.created_at) DESC`,
    [userId]
  );
  return rows.map((r) => ({
    ...
    partner: r.partner_id
      ? publicUser({
          id: r.partner_id, username: r.partner_username, display_name: r.partner_display_name,
          photo: r.partner_photo, gender: r.partner_gender, bio: r.partner_bio,
          youtube_link: r.partner_youtube, birth_date: r.partner_birth_date,
          phone_hash: r.partner_phone_hash, is_premium: r.partner_premium
        })
      : null,
    lastMessage: r.lm_ts != null
      ? { type: r.lm_deleted ? 'text' : r.lm_type, text: r.lm_deleted ? null : r.lm_text, ts: Number(r.lm_ts), mine: r.lm_sender_id === userId }
      : null,
    unreadCount: Number(r.unread),
    updatedAt: Number(r.last_message_at || r.created_at)
  }));
}
...
async function deleteMessageForEveryone(messageId, userId) {
  ...
  // If this was a stored voice message, remove the object too (best effort).
  if (row.audio && !String(row.audio).startsWith('data:')) {
    media.deleteObject(String(row.audio)).catch(() => {});
  }
...
/**
 * friendStatusBetween for a whole list in ONE query (search results,
 * contact matches used to do one query per row).
 */
async function friendStatusBatch(viewerId, otherIds) {
  const ids = [...new Set(otherIds.filter(Boolean))];
  const out = new Map();
  if (ids.length === 0) return out;
  const rows = await query(
    `SELECT * FROM friend_requests
     WHERE (from_user = $1 AND to_user = ANY($2::text[]))
        OR (to_user = $1 AND from_user = ANY($2::text[]))`,
    [viewerId, ids]
  );
  for (const id of ids) out.set(id, 'none');
  for (const r of rows) {
    const other = r.from_user === viewerId ? r.to_user : r.from_user;
    if (r.status === 'accepted') out.set(other, 'friends');
    else if (r.status === 'pending') {
      out.set(other, r.from_user === viewerId ? 'pending_outgoing' : 'pending_incoming');
    }
  }
  return out;
}
...
/** Friends list in ONE query (the route used to fetch every friend row
 *  individually). */
async function listFriendUsers(userId) {
  const rows = await query(
    `SELECT u.* FROM friend_requests fr
     JOIN users u ON u.id = CASE WHEN fr.from_user = $1 THEN fr.to_user ELSE fr.from_user END
     WHERE (fr.from_user = $1 OR fr.to_user = $1) AND fr.status = 'accepted'
     ORDER BY fr.responded_at DESC NULLS LAST`,
    [userId]
  );
  return rows.filter(u => !u.is_banned);
}
...
async function setLastCountry(userId, country) {
  if (!country) return;
  // One UPDATE per connect was fine at prototype scale; at high connection
  // churn it's a lot of pointless writes — only write when it changed.
  const res = await pool.query(
    `UPDATE users SET last_country = $1 WHERE id = $2 AND last_country IS DISTINCT FROM $1`,
    [country, userId]
  );
  if (res.rowCount > 0) await cacheDel(userKey(userId));
}
...
async function updateProfile(userId, { displayName, bio, photo, youtubeLink } = {}) {
  ...
  if (typeof photo === 'string' && photo.startsWith('data:image/')) {
    // Legacy inline path (client-compressed). New clients upload via
    // /api/media/upload-url and send a storage key instead.
    if (photo.length > PROFILE_PHOTO_MAX_CHARS) throw new Error('Photo is too large.');
    photoData = photo;
  } else if (typeof photo === 'string' && photo.startsWith('media/photo/')) {
    // Storage key — must live in this user's own namespace.
    if (!photo.startsWith(`media/photo/${userId}/`)) throw new Error('Invalid photo reference.');
    if (!media.isValidKey(photo, 'photo')) throw new Error('Invalid photo reference.');
    photoData = photo;
  }
...
/** getReviewsForUser for a batch of users (recent-partners used to do one
 *  query per partner). Returns Map<userId, reviews[]>. */
async function getReviewsForUsers(reviewedIds, { limit = 50 } = {}) {
  const ids = [...new Set(reviewedIds.filter(Boolean))];
  const out = new Map();
  if (ids.length === 0) return out;
  const rows = await query(
    `SELECT r.*, u.username AS reviewer_username, u.display_name AS reviewer_display_name, u.photo AS reviewer_photo
     FROM reviews r JOIN users u ON u.id = r.reviewer_id
     WHERE r.reviewed_id = ANY($1::text[])
     ORDER BY r.updated_at DESC`,
    [ids]
  );
  for (const row of rows) {
    const list = out.get(row.reviewed_id) || [];
    if (list.length < limit) {
      list.push({
        id: row.id, rating: row.rating, tag: row.tag, comment: row.comment || '',
        createdAt: Number(row.created_at), updatedAt: Number(row.updated_at),
        reviewer: { id: row.reviewer_id, username: row.reviewer_username, displayName: row.reviewer_display_name || row.reviewer_username, photoUrl: row.reviewer_photo || null }
      });
      out.set(row.reviewed_id, list);
    }
  }
  return out;
}

/** getReviewSummary for a batch of users. Returns Map<userId, summary>. */
async function getReviewSummaries(reviewedIds) {
  const ids = [...new Set(reviewedIds.filter(Boolean))];
  const out = new Map();
  if (ids.length === 0) return out;
  const rows = await query(
    `SELECT reviewed_id, COUNT(*) AS cnt, COALESCE(AVG(rating), 0) AS avg,
            COUNT(*) FILTER (WHERE tag = 'genuine') AS genuine_count,
            COUNT(*) FILTER (WHERE tag = 'fake') AS fake_count,
            COUNT(*) FILTER (WHERE tag = 'suspicious') AS suspicious_count
     FROM reviews WHERE reviewed_id = ANY($1::text[]) GROUP BY reviewed_id`,
    [ids]
  );
  for (const id of ids) {
    out.set(id, { count: 0, average: 0, tagCounts: { genuine: 0, fake: 0, suspicious: 0 } });
  }
  for (const row of rows) {
    out.set(row.reviewed_id, {
      count: Number(row.cnt),
      average: Math.round(Number(row.avg) * 10) / 10,
      tagCounts: { genuine: Number(row.genuine_count), fake: Number(row.fake_count), suspicious: Number(row.suspicious_count) }
    });
...
module.exports = {
  ...
  findOrCreateConversation, getConversation, listConversationsForUser, listConversationsDetailed, otherUserId,
  ...
  friendStatusBetween, friendStatusBatch, areFriends, listIncomingRequests, listFriendIds, listFriendUsers,
  ...
  verifyPassword, hashPassword, flushPendingWrites,
  savePushSubscription, removePushSubscription, getPushSubscriptionsForUser,
  hasChattedWith, upsertReview, getReviewsForUser, getReviewsForUsers, getReviewSummary, getReviewSummaries, getMyReviewFor, reportReview,
